...
const SLUG = "run" as const;
const PATH = "/commands/run";

export async function generateMetadata({
  params,
}: PageProps<"/[locale]/commands/run">): Promise<Metadata> {
...
export default async function RunCommandDoc({
  params,
}: PageProps<"/[locale]/commands/run">) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
